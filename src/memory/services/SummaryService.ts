...
export class SummaryService extends BaseService {
     ...
     */
    private async summarizeMessages(messages: SummaryReadyMessage[]): Promise<string> {
        try {
...
Summary:`;
     ...
     */
    private async prepareMessagesForSummary(conversationId: string): Promise<SummaryReadyMessage[]> {
        try {
            // Get filtered messages using existing logic
            const messages = await this.getMessageProcessor().getMessagesForSummary(conversationId);
    ...
    async generateGlobalSummary(
        ...
        try {
            ...
            const allPreferences = recentSummaries.reduce<Record<string, number>>((acc, summary) => {
                if (summary.metadata?.userPreferences) {
                    Object.entries(summary.metadata.userPreferences).forEach(([key, value]) => {
                ...
                VECTOR_INDICES.CONVERSATIONS.name,
                {
                    ...
                    metadata: {
                        ...
                        timeRange: {
                            ...
                            endTime: recentSummaries[recentSummaries.length - 1].timestamp.toISOString()
                        }
                    }
                }
            );
