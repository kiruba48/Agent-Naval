...
// Convert AIMessage to OpenAI's ChatCompletionMessageParam
function convertToOpenAIMessage(
  message: AIMessage
): ChatCompletionMessageParam {
  if (message.role === 'tool') {
    // Handle tool messages as assistant messages with function call results
    return {
      role: 'assistant',
      content: message.content ?? '',
      function_call: message.name
        ? {
            name: message.name,
